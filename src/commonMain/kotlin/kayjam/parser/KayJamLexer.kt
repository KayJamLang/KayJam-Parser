...
class KayJamLexer(val originalInput: String, val source: String = "unknown"){
    ...
    fun moveAhead(needType: Token.Type? = null): Token {
        ...
        if(finished) throw KayJamLexerException(this, "Already finished: EOF")

        cleanup()
        if(findNextToken(needType)){
            while (currentToken?.type==Token.Type.TK_NEW_LINE||currentToken?.type==Token.Type.COMMENT){
                position = Position(position.line+1, 1)

                if (input.isEmpty()) {
                    finished = true
                    throw KayJamLexerException(this, "Already finished: EOF")
                }
        ...
        finished = true
        if(input.isNotEmpty()){
            throw KayJamLexerException(this, "Unexpected symbol: '" + input[0] + "'")
        }
    ...
    private fun cleanup() {
        ...
        if (charsToDelete > 0) {
            ...
            input = input.delete(0, charsToDelete)
        }
    }
    ...
    private fun findNextToken(needType: Token.Type? = null): Boolean {
        ...
        if(tokenScan!=null&&tokenType!=null){
            currentToken = Token(tokenType, tokenScan.groups.lastNotNull()!!.value, position)
            input = input.delete(0, tokenScan.value.length)
